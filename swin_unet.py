...
class WindowAttention(nn.Module):
    ...
    def window_partition(self, x: torch.Tensor) -> torch.Tensor:
        _, H, W, _ = x.shape
        #print('before window partition', x.shape)
        ...
        #print('after window partition',x.shape)
        return x
    ...
    def forward(self, x):
        ...
        x = func.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b))
        ...
        #print('after window partition', x.shape)
        #if mask is not None:
        #    print('mask:', mask.shape,'input x:',x.shape)
        Bn, Mh, Mw, _ = x.shape
        ...
        x = attn @ v
        #print('before last rearrange',x.shape, Mh,Mw, H)
...
class SwinUnet(nn.Module):
    ...
    def skip_connection(self):
        ...
        for i in range(self.num_layers - 1):
            ...
            dim = self.embed_dim * 2 ** (self.num_layers - 2 - i)
            layer = nn.Conv2d(in_channels=dim * 2, out_channels=dim , kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            #print('before layer '+str(i),x.shape)
            x_save.append(x)
            x = layer(x)
            #print('after layer '+str(i),x.shape)

        x = self.first_patch_expanding(x)
        #print('after first patch expanding ',x.shape)

        for i, layer in enumerate(self.layers_up):
            #print('before upsampling stage ',i, x.shape, x_save[len(x_save) - i - 2].shape)
            x = torch.cat([x, x_save[len(x_save) - i - 2]], -1)
            #print('upsampling after concat stage ',i,x.shape)
            x = x.permute(0, 3, 1, 2)
            x = self.skip_connection_layers[i](x)
            x = x.permute(0, 2, 3, 1)
            #print('upsampling after skip con stage ',i,x.shape)
            x = layer(x)
            #print('upsampling after up layer stage ',i,x.shape)
